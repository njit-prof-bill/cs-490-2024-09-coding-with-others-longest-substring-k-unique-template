def longest_substring_k_unique(s: str, k: int) -> int:
    ...
    if length_s <k: #can't have string less than k unique characters
        return 0

    length=0 #current length of substr
    for i in range (length_s-k): #looping through the string to find max substring
        j=0 #incrementing by one, looking at the next character
        while len(letters)<=k: #k unique characters not reached
            if i+j >= length_s: #end of string
